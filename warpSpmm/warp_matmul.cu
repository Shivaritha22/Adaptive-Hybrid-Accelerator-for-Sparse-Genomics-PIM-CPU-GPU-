...
void cpu_matmul(float *X, float *W, float *Y, int rows, int inner, int cols) {
...
__global__ void warp_matmul_kernel(float *X, float *W, float *Y,
                                   int rows, int inner, int cols) {
    int row = blockIdx.y;
    int col = blockIdx.x;
    int lane_id = threadIdx.x;

    if (row >= rows || col >= cols) return;  // safety
